    ...
    fun showBiometricPrompt(fragmentActivity: FragmentActivity) {
        if (isBiometricBlocked) {
            ...
            if (System.currentTimeMillis() < (blockTime + blockMillis)) { //블럭 상태
                Toast.makeText(fragmentActivity.applicationContext, "시도 횟수가 너무 많습니다. 나중에 다시 시도하세요.", Toast.LENGTH_SHORT).show()
                return
            } else {
        ...
        val promptInfo = BiometricPrompt.PromptInfo.Builder().apply {
            setTitle(fragmentActivity.applicationContext.getString(R.string.prompt_title))
            setSubtitle(fragmentActivity.applicationContext.getString(R.string.prompt_subtitle))
            setDescription(fragmentActivity.applicationContext.getString(R.string.prompt_description))
            setNegativeButtonText(fragmentActivity.applicationContext.getString(R.string.prompt_negative_button))
        ...
        val biometricPrompt = BiometricPrompt(fragmentActivity, executor,
            object : BiometricPrompt.AuthenticationCallback() {
                override fun onAuthenticationError(errCode: Int, errString: CharSequence) { //지문 인식 ERROR
                    ...
                    when(errCode) {
                        ...
                        BiometricPrompt.ERROR_NO_BIOMETRICS -> {
                            ...
                            Toast.makeText(fragmentActivity.applicationContext, "$errString", Toast.LENGTH_SHORT).show()
                            showSecuritySettingDialog(fragmentActivity)
                        }
                        else -> {
                ...
                override fun onAuthenticationSucceeded(result: BiometricPrompt.AuthenticationResult) { //"지문 인식 성공"
                    ...
                    Toast.makeText(fragmentActivity.applicationContext, "생체인식 성공!!!", Toast.LENGTH_SHORT).show()
